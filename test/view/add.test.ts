...
import getAddToView from "../../src/view/add";

test("returns correct result with one key", () => {
  const data = new Map();
  const addToView = getAddToView(data, ["shift"], (item) => item);
  addToView(tasks, []);
...
test("returns correct result with multiple keys", () => {
  const data = new Map();
  const addToView = getAddToView(data, ["shift", "type"], (item) => item);
  addToView(tasks, []);
...
test("returns correct result with multiple keys and ignores missing values mixed with present values", () => {
  const data = new Map();
  const addToView = getAddToView(data, ["type", "employee"], (item) => item);
  addToView(tasks, []);
  const groupedByShiftAndType = toObject(data, 2);
...
test("adds correcty with a nested key", () => {
  const data = new Map();
  const addToView = getAddToView(
    data,
    ["age", "address.zipCode"],
...
test("ignores items with missing keys", () => {
  ...
  addToView(studentsWithMissingKeys, []);
  const studentsByAgeAndZipCode = toObject(data, 2);
...
test("cannot pollute prototype with __proto__ property", () => {
  ...
  const addToView1 = getAddToView(
    new Map(),
    ["__proto__", "name", "age"],
    (item) => item
  );
  const addToView2 = getAddToView(
    new Map(),
    ["name", "__proto__", "age"],
    (item) => item
  );
  const addToView3 = getAddToView(
    new Map(),
    ["name", "age", "__proto__"],
    (item) => item
  );
  const addToView4 = getAddToView(
    new Map(),
    ["__proto__.toString", "age"],
    (item) => item
  );
  const addToView5 = getAddToView(new Map(), ["__proto__"], (item) => item);
  const addToView6 = getAddToView(
    new Map(),
    ["__proto__.toString"],
    (item) => item
  );
  addToView1(studentsWithMissingKeys, []);
  addToView2(studentsWithMissingKeys, []);
  addToView3(studentsWithMissingKeys, []);
  addToView4(studentsWithMissingKeys, []);
  addToView5(studentsWithMissingKeys, []);
  addToView6(studentsWithMissingKeys, []);
  const a: any = {};
...
test("cannot pollute prototype with __proto__ values", () => {
  ...
  const addToView1 = getAddToView(new Map(), ["age", "name"], (item) => item);
  const addToView2 = getAddToView(new Map(), ["age.name"], (item) => item);
  const addToView3 = getAddToView(new Map(), ["age.__proto__"], (item) => item);
  const addToView4 = getAddToView(
    new Map(),
    ["age.name", "address.street"],
    (item) => item
  );
  const addToView5 = getAddToView(
    new Map(),
  ...
  const addToView7 = getAddToView(
    new Map(),
    ["__proto__", "name", "age"],
    (item) => item
  );
  const addToView8 = getAddToView(
    new Map(),
    ["name", "__proto__", "age"],
    (item) => item
  );
